...
int main (int argc, char **argv, char **envp)
{
  ...
  char *options = "1r:d:enfDp:b:A:BiG:S:F:R:C:";
  char *server_mbox, *s;
  int c, port;
  ...
  while ((c = _getopt (argc, argv, options)) != EOF)
    switch ((char) c) {
      ...
      case 'G': fprintf(stderr, "listproc: option -G is no longer used.\n"); 
		break;
      case 'S': fprintf(stderr, "listproc: option -S is no longer used.\n"); 
		break;
      case 'f': afd_fui_delivery = TRUE; break;
      case 'p': send_published_lists = TRUE; port = atoi (optarg); break;
      case 'F': mailforwardf = mystrdup (optarg); break;
      case 'R': strcpy (requests_file, optarg); break;
      case 'C': replyf = mystrdup (optarg); break;
      case 'B': process_batch = TRUE; break;
      ...
      case 'd':
        ...
        for (i = 0; i < MAX_COMMANDS; ++i) {
          notok &= (((j = strncmp (optarg, commands[i].name, strlen (optarg)))
                     != 0) ? 1 : 0);
          if (!j) {
            if (strlen (optarg) != strlen (commands[i].name))
              ++k;
            if (!commands[i].mask)
              fprintf (stderr, "listproc: invalid command line option -%c for \
request '%s'\n", c, optarg),
                exit (3);
            if (c == 'r')
              ...
              batched_commands |= commands[i].mask;
          }
        }
        if (notok)
          fprintf (stderr, "listproc: Unrecognized request '%s'\n", optarg),
            exit (3);
        if (k > 1) /* ambiguous command */
          fprintf (stderr, "listproc: Ambiguous request '%s'\n", optarg),
            exit (3);
        break;
      case ':':
        fprintf (stderr, "listproc: Option '%c' requires an argument.\n",
                 optopt);
        exit (3);
      case '?':
      default:
  ...
  lpl_unlock(LPL_GLOBAL_SYSFILES,NULL);
    ...
    if ((report = fopen (REPORT_SERVER, "a")) == NULL)
      fprintf (stderr, "listproc: Unable to open %s\n", REPORT_SERVER),
      exit (1);
    lpfile_chmod(REPORT_SERVER, 0666);
  }
#endif
...
%d, %s", errno, sys_errlist[errno]), TRUE),
  ...
  if (afd_fui_delivery) {	/* Precedence over anything else */
    report_progress (report, "Performing AFD/FUI...", TRUE);
    if (!afd_fui_deliver (DEFAULT_ARCHIVE, NULL)) {
  ...
  else if (send_published_lists) {	/* Send to global-update-server */
    report_progress (report, "Sending published lists...", TRUE);
    send_pub_lists (port);
    goto skip;
  }

  s=NULL;
  if (process_batch) /* Select file to process: batch or requests */
    strcpy (requests_file, s=BATCH_FILE);
  else if (!interactive)
    strcpy (requests_file, s=SERVER_MAIL_FILE);
  if(s != NULL) free(s);

#ifdef GO_INTERACTIVE
  if (requests_file[0] == EOS)
    fprintf (stderr, "\nmain(): -R option required\n"),
    gexit (3);
#endif

  do {
    if (!stat (requests_file, &stat_buf) && stat_buf.st_size > 0) {

	  /* lock the requests file and copy it to a temporary location */
      if (!interactive) {
		lpl_lock(LPL_WRITE,LPL_GLOBAL_REQUESTS,NULL);
		cp (requests_file, mail_copy);
		unlink(requests_file);
		lpfile_touch(requests_file);
		lpfile_chmod(requests_file,0666);
		lpl_unlock(LPL_GLOBAL_REQUESTS,NULL);
	  }

	  /* copy the requests to mbox */
      if (!process_batch) {
		if(interactive) cat_append(requests_file, server_mbox);
		else cat_append(mail_copy, server_mbox);
		echo_append ("", server_mbox);
	  }

      OPEN_FILE(mail, (!interactive ? mail_copy : requests_file), "r", "main");
      if (process_batch)
		report_progress (report, PROCESSING_BATCH, FALSE);
      else
        report_progress (report, NEW_ARRIVAL, FALSE);
      distribute (mail, (void (*)(char *, char *, BOOLEAN)) process_message,
				  report, NULL, NULL, NULL, NULL, NULL, NULL, NULL, TRUE, NULL);
      fclose (mail);  /* Done */
      if (!interactive) {
	  ...
	  /* 
      if (!interactive) {
		OUT_OF_CRITICAL_SECTION ("main", SEM_DLVR_MAIL, NULL);
      }*/
    }
 ...
 skip:
  free_remote (&rlists);
  if (restart_sys)
    gexit (7); /* Exit status of 7 signifies a restart request */
  else if (reread_config)
    gexit (17);	/* Tell serverd to reread the config file and reinitialize */
  gexit (0);
}

/*
...
#define ERR_XLP_VERSION		0x1

void process_message (char *sender, char *linecopy, BOOLEAN address_ok)
{
  char line [MAX_LINE];          /* ... from the current message */
  ...
  char received [MAX_LINE];
  char precedence [MAX_LINE];	   /* Holds the Precedence: */
  char match [MAX_LINE];
  char error [MAX_LINE];
  char *symptoms = NULL;
  char sum [MAX_LINE];
  char *at = NULL;
  char *s, *ss = NULL, *ss1;
  char head_cookie[MAX_LINE];        /* sub w/ confirm cookie */
  char body_cookie[MAX_LINE];
  char *pos;                    /* for sub w/ confirm */
  static char *sumf;
  static char *msg_no;
  static char *checksum_prog, *zero_sum;
  FILE *f;
  FILE *msg_nof     = NULL; /* Last message count */
  FILE *msg;
  FILE *msg2;
  FILE *headers;
  BOOLEAN loop = FALSE;
  ...
  BOOLEAN done;
  BOOLEAN fake_mail = TRUE;
  BOOLEAN mailer_daemon;
  BOOLEAN susp_subject;
  BOOLEAN ignore_msg = FALSE;
  BOOLEAN is_invalid;
  BOOLEAN all_invalid;
  BOOLEAN stop_msg2;
  BOOLEAN to_owners = FALSE;
  BOOLEAN keep_line;
  BOOLEAN perform_body_checks;
  BOOLEAN first_line_checked;
  BOOLEAN ignored;
  BOOLEAN cookie_found = FALSE;    /* sub w/ confirm cookie */
  int errmask = 0;
  extern char *getenv();
  ...
  if (!checksum_prog)
    if (getenv ("MD5"))
      checksum_prog = "md5",
      zero_sum = "^d41d8cd98f00b204e9800998ecf8427e$";
    else
      checksum_prog = "sum",
      zero_sum = "^0+$";
  ...
  if (!msg_no)
    msg_no = MSG_NO;
  if (original_x)
    free ((char *) original_x),
    original_x = NULL;
  if (original_mime)
    free ((char *) original_mime),
    original_mime = NULL;
  OPEN_FILE (msg, msgf, "w", "process_message");
  OPEN_FILE (msg2, msgf2, "w", "process_message");
  OPEN_FILE (headers, headerf, "w", "process_message");
  listid = (list *) -1;
  peer_server_request = FALSE;
  line[0] = message_id[0] = id_copy[0] = precedence[0] = received[0] = 
    original_sender[0] = senders_subject[0] = 
      _line[0] = RESET (sender_copy);
  strcpy (sender_copy, sender);  /* We do not like converting the actual */
  upcase (sender_copy);          /* address to upper case */


  lpl_lock(LPL_READ,LPL_GLOBAL_REQ_ID,NULL);
  if ((msg_nof = fopen (msg_no, "r")) != NULL)
    fscanf (msg_nof, "%d\n", &request_no),
    fclose (msg_nof);
  report_progress (report, (s = tsprintf ("Request #%04d:%s\n", ++request_no,
					  (*sender != EOS ? sender : "\"\""))),
		   FALSE);
  free ((char *) s);
  OPEN_FILE (msg_nof, msg_no, "w", "process_message");
  fprintf (msg_nof, "%d\n", request_no);
  fclose (msg_nof);
  ...
  while (!feof (mail) && line[0] != '\n') { /* Skip to beginning of message */
    ...
    /* check to see if the line contains a confirmation */
    if( (pos=_strstr(line, CONF_COOKIE_IDENTIFIER)) != NULL) {
      cookie_found = TRUE;
      pos += strlen(CONF_COOKIE_IDENTIFIER);
      sscanf(pos, "%s", head_cookie);
    }

    if (re_strcmp (SUBJECT, line, match) > 0) {
      char *line_copy = mystrdup (line);
      strcpy (senders_subject, line_copy + strlen (match));
      sprintf (line_copy, "%s", line_copy + strlen (match));
      strcpy (match, "\\1");
      if (re_strcmp (PEER_SERVER_REQUEST, line_copy, NULL) > 0)
    ...
    else if (re_strcmp (FROM, line, match) > 0) {
      strcpy (original_sender, line + strlen (match)); /* Save From: */
      original_sender [LAST_CHAR (original_sender)] = EOS; /* \n -> \0 */
      ss1 = NULL;
      ss = original_sender;
	}

    else if (re_strcmp (SENDER, line, match) > 0) {
      strcpy (original_senderaddr, line + strlen (match));
      original_senderaddr [LAST_CHAR (original_senderaddr)] = EOS;
      ss1 = NULL;
      ss = original_senderaddr;
	}

    else if (re_strcmp (MESSAGE_ID, line, match) > 0) {
      strcpy (message_id, line + strlen (match));
      message_id [LAST_CHAR (message_id)] = EOS; /* \n -> \0 */
      strcpy (id_copy, message_id);
      lplog_message(NULL,LG_MESS,"Processing Message-Id: %s", id_copy);
      upcase (id_copy);

      /* Check for mail loop using Message-Id: */

      if(message_id_match(NULL,id_copy)) {
         APPEND_TO_STR (symptoms, (s = tsprintf ("- A message with Message-Id: %s\n\
has already already been processed.\n", id_copy)));
         free ((char *) s);
         loop = TRUE;
         goto next;
      } 
      lpl_lock(LPL_WRITE,LPL_GLOBAL_SYSFILES,NULL);
      OPEN_FILE (message_ids, message_idsf, "a", "process_message");
	  ...
	  else if (re_strcmp ("^[a-zA-Z][a-zA-Z0-9-]+:[ \t]*", line, NULL) == 0 && ss &&
			   (int) (strlen (ss) + strlen (line)) <= MAX_LINE - 2) {
		if (*ss == EOS) {
		  if (ss1 && ss1 == original_x)
			original_x = (char *) realloc ((char *) original_x,
										   (strlen (ss1) + strlen (line) + 2) *
										   sizeof (char)),
			  ss = original_x;
		  else if (ss1 && ss1 == original_mime)
			original_mime = (char *) realloc ((char *) original_mime,
											(strlen (ss1) + strlen (line) + 2) *
											sizeof (char)),
			ss = original_mime;
      }
      if (!ss)
		report_progress (report, "\nprocess_message(): realloc() failed",
						 TRUE),
		  gexit (11);
      sprintf (ss + strlen (ss), "\n%s", line);
      ss [LAST_CHAR (ss)] = EOS;
      if (ss == original_x)
		ss = original_x + strlen (original_x),
		  ss1 = original_x;
      else if (ss == original_mime)
		ss = original_mime + strlen (original_mime),
		  ss1 = original_mime;
      else
		ss = ss1 = NULL;
    }

    else { /* Check if precious or unwanted header line */
      ss1 = NULL;
      ss = NULL; /* ~~~ We have no way of saving multiple lines of precious headers */
      if(glob_saved_headers!=NULL) {
         int i;
         for(i=0; i<glob_saved_headers->filled; i++) {
            if(re_strcmp(glob_saved_headers->data[i], line, NULL) > 0) {
               keep_line = FALSE;
               goto next;
            }
         }
      ...
      pheader = sys.header;
      while (pheader) {
		if (re_strcmp (pheader->line, line, NULL) > 0 &&
			!pheader->keep) {
		  keep_line = FALSE;
		  goto next;
		}
...
/* fake mail check removed from here - hga 7/19/99 */

/* commented ignore low precedence check removed from here - hga 7/19/99 */

      if (re_strcmp (XDASH, line, match) > 0) {
		if (!original_x) {
		  if (! (original_x = (char *) malloc ((strlen (line) + 1) * sizeof (char))))
		  ...
		  *original_x = EOS;
		  ...
		  if (! (original_x = (char *) realloc ((char *) original_x,
												(strlen (original_x) + 
												 strlen (line) + 2) * 
												sizeof (char))))
			report_progress (report, "\nprocess_message(): realloc() failed",
		...
		strcat (original_x, line);
		original_x [LAST_CHAR (original_x)] = EOS;
		ss = original_x + strlen (original_x);
		ss1 = original_x;
      }
	  
      else if (re_strcmp (MIME, line, match) > 0) {
		if (!original_mime) {
		  if (!(original_mime = (char *) malloc ((strlen (line) + 1) * sizeof (char))))
			report_progress (report, "\nprocess_message(): malloc() failed",
							 TRUE),
			  gexit (11);
		  *original_mime = EOS;
		  ...
		  if (! (original_mime = (char *) realloc ((char *) original_mime,
												   (strlen (original_mime) + 
													strlen (line) + 2) * 
												   sizeof (char))))
			report_progress (report, "\nprocess_message(): realloc() failed",
							 TRUE),
			  gexit (11);
		  strcat (original_mime, "\n");
		}
		strcat (original_mime, line);
		original_mime [LAST_CHAR (original_mime)] = EOS;
		ss = original_mime + strlen (original_mime);
		ss1 = original_mime;
      }
	  
	  /* Always pass on all unknown headers */
	  /* 
      else if (sys.header) /* if FALSE, pass on all unknown headers * /
		keep_line = FALSE;
		*/
    }
  ...
  /* Check for mail loop using Message-Id: and X-Listprocessor-Version: in
     the body */
  stop_msg2 = first_line_checked = FALSE;
  perform_body_checks = TRUE;
  while (!feof (mail) && 
	 (interactive ||	/* So we can archive mbox-format files */
	  strncmp (_line, START_OF_MESSAGE, strlen (START_OF_MESSAGE)))) {
    ...
    if (!stop_msg2)
      fputs (_line, msg2);
    RESET (_line);
    fgets (_line, MAX_LINE - 2, mail);
    RESET (request);
    sscanf (_line, " %s", request);
    if (!interactive && !first_line_checked && request[0] != EOS) {
      first_line_checked = TRUE;
      upcase (request);
      if ((!strncmp (request, "PUT", strlen (request)) ||
	   !strncmp (request, "ARCHIVE", strlen (request)) ||
	   !strncmp (request, "INTERNAL_NOTIFY", strlen (request))) &&
	  (int) strlen (request) > 2)
	perform_body_checks = FALSE;
    }
    if (!to_owners && re_strcmp ("^NOTIFY_OWNERS", _line, NULL) > 0)
      to_owners = TRUE;
    if (!interactive && perform_body_checks && !to_owners) {
      strcpy (__line, _line);
      upcase (__line);
      if (_strstr (__line, "UNSENT MESSAGE FOLLOWS") ||
		  _strstr (__line, "REJECTED MESSAGE") ||
		  _strstr (__line, "TRANSCRIPT OF MESSAGE FOLLOWS") ||
		  _strstr (__line, "RETURNED MAIL") ||
		  re_strcmp ("^X?-?4?0?0?-?[a-zA-Z0-9-]+:[ \t]*", __line, NULL) > 0 ||
		  re_strcmp ("^\\>FROM[ \t]", __line, NULL) > 0)
		stop_msg2 = TRUE;
      strcpy (match, "\\1");
      if (re_strcmp (MESSAGE_ID, _line, match) > 0) {
		strcpy (id_copy, _line + strlen (match));
		id_copy [LAST_CHAR (id_copy)] = EOS; /* \n -> \0 */
		upcase (id_copy);

		/* lpl_lock(LPL_READ,LPL_GLOBAL_SYSFILES,NULL);
		if (message_ids = fopen (message_idsf, "r")) {
		  if (ignore_sender (message_ids, id_copy, report, TRUE)) { */
		if( message_id_match(NULL,id_copy) ) {
		  APPEND_TO_STR (symptoms, (s = tsprintf ("- A message with Message-Id: %s (appearing\n\
in the body) has already been processed.\n", id_copy)));
		  free ((char *) s);
		  loop = TRUE;
		  /* }
		  fclose (message_ids); */
      ...
      else if (re_strcmp (LISTPROC_ID, _line, NULL) > 0) {
		if (!(errmask & ERR_XLP_VERSION)) {
		  APPEND_TO_STR (symptoms, (s = tsprintf ("- Found ListProcessor(tm) \
identification tag in message body: %s", _line)));
		  free ((char *) s);
		  errmask |= ERR_XLP_VERSION;
		}
		loop = TRUE;
      }
    }
  ...
  /*
   *  Set the file position to the beginning of the next message, and
   *  store that value.
   */
  if (!feof (mail)) {
	fseek(mail, -strlen(_line), SEEK_CUR);  
    global_mail_offset = ftell (mail);
  }
  else
    global_mail_offset = -1;

  if (!interactive && perform_body_checks) {
	MMAP_FILE *mf;
	char *temp;
	void *md5sum;

	mf = lpfile_mmap_open(msgf2,"r");
	if(mf != NULL && mf != (MMAP_FILE*) -1) {
	  lpfile_mmap_endstring(mf);

	  /* start the checksum */
	  md5sum = md5_start_digest();
	  md5_add_to_digest(md5sum,sender,strlen(sender));
	  md5_add_to_digest(md5sum,mf->mmap_start,strlen(mf->mmap_start));
	  temp = md5_finish_digest(md5sum);
	  strcpy(sum,temp);
	...
	/* lpl_lock(LPL_READ,LPL_GLOBAL_SYSFILES,NULL);*/
	if( checksum_match(NULL,sum) ) {
	  APPEND_TO_STR (symptoms, (s = tsprintf ("- The body of the message has a checksum \
matching those messages already\nprocessed.\n")));
	  free ((char *) s);
	  loop = TRUE;
    }
	/* lpl_unlock(LPL_GLOBAL_SYSFILES,NULL); */
  ...
  if (!address_ok) {
    RESET (request);
    NOTIFY_OF_INVALID_USER_ADDRESS (sys.manager, sender);
    one_rejection = TRUE;
  }

  /* lplog_message(NULL,LG_MESS,"Made it here 1"); */

  mailer_daemon = strinstr (sys.mailer_daemon, sender_copy) ||
/* 8.0: (original_sender[0] != EOS && strinstr (sys.mailer_daemon, original_sender)) || */
    *sender == EOS;
  /* lplog_message(NULL,LG_MESS,"Made it here 2"); */

  susp_subject = (senders_subject[0] != EOS &&
		  strinstr (sys.susp_subject, senders_subject));
  if (mailer_daemon) {
    APPEND_TO_STR (symptoms, (s = tsprintf ("- Suspicious address: %s\n",
	     (*sender != EOS ? sender : "\"\""))));
    free ((char *) s);
    did_action = loop = TRUE;
  }
  if (susp_subject) {
    APPEND_TO_STR (symptoms, "- Suspicious subject.\n");
    did_action = loop = TRUE;
  }
  ...
  if (!to_owners && loop && !interactive) {
    NOTIFY_MANAGER_OF_MSG_IGNORED (symptoms, headerf, msgf, "process_message",
				   ccignore);
  }
  /* 
   * Check for & deal with command confirmation cookies
   */ 
  else if(find_conf_cookie(msgf,body_cookie) || cookie_found) {
    do_confirmed_request(sender, msgf, headerf, 
			 body_cookie, head_cookie);
    did_action = TRUE;
  }
  else { /* Isolate request and call action() */
    char _line [MAX_LINE];
/* No need; the file is written by listproc on PUT requests from manager only
#ifndef NO_IPC_SUPPORT
    IN_CRITICAL_SECTION ("process_message", SEM_SYSFILES, NULL);
#endif
*/
    check_aliases (global_aliasesf, sender);
/*
#ifndef NO_IPC_SUPPORT
    OUT_OF_CRITICAL_SECTION ("process_message", SEM_SYSFILES, NULL);
#endif
*/
    strcpy (sender_copy, sender);
    done = did_action = ignored = FALSE;
    all_invalid = TRUE;
    OPEN_FILE (body, msgf, "r", "process_message");
    while (!feof (body)) {
	  ...
	  request_line = lpconfig_read_line(body,LPC_INITIAL_COMMENTS);
	  if(request_line==NULL || request_line[0]==EOS)
		continue;

	  /* remove leading blanks, and terminate at bad chars */
	  clean_request(request_line);

	  /* truncate at 1022 to be nice to older routines */
	  if(strlen(request_line) > 1022)
		request_line[1022] = EOS;

	  /* read the command */
	  request[0] = EOS;
	  sscanf (request_line, "%s ", request);
	  if(request[0] == EOS)
		continue;
      upcase (request);

	  /* copy the first part of the line */
	  strncpy(_line,request_line,MAX_LINE-2);
	  _line[MAX_LINE-1] = EOS;
	  upcase(_line);


      strcpy (sender, sender_copy);  /* Could have been aliased by prev req */
      if (ignored)	/* User was ignored */
		break;
      else if (!strcmp (request, START_OF_SIGNATURE) || strinstr (THANKS, request))
		done = TRUE; /* End of requests, start of .signature */
      else if (request[0] != EOS && !one_rejection && !loop && !done) {
		if (sys.ignored_requests[0] == EOS ||
			!re_strcmp (sys.ignored_requests, _line, NULL))
		  is_invalid = FALSE,
			did_action |= action (request_line, request, sender, 
								  FALSE, &is_invalid, &ignored, body),
			all_invalid &= is_invalid;
      }
      else {
		int i;
		is_invalid = FALSE;
    ...
    fclose (body);
    if (!interactive && all_invalid && re_strcmp (zero_sum, sum, NULL) <= 0 &&
	(msg = fopen (checksumsf, "a"))) /* Save checksum */
      fprintf (msg, "%s %s\n", sum, sender),
      fclose (msg);
  }
  if (!did_action && !one_rejection && !loop) {
    create_header (&f, mailforwardf, sys.server.address, sender,
		   "No requests found", COPY_OWNER (ccerrors), INVALID_REQ,
		   FALSE, FALSE);
    report_progress (report, "[no requests found]", TRUE);
    if (!interactive) {
      fprintf (f, "No requests were found or recognized in your message. It \
is possible\nyour message was intended for a person or a list, or none of its \
contents\nwere recognized by this automated system as requests.\nIf you need \
to talk to a person, please send mail to %s.\n\
To get general help on Listproc, please send the command:\n\n\t\t\t\tHELP\n\n\
in the body of the message to %s.\
\n--------------------------------------\
-----------------------------------------\n", sys.manager, sys.server.address);
      fclose (f);
      cat_append (headerf, mailforwardf);
      cat_append (msgf, mailforwardf);
      APPEND_TELNET ("process_message");
      DELIVER_MAIL (sender, COPY_OWNER (ccerrors));
    }
    else
      fprintf (f, "No request issued.\n"),
      fclose (f);
  }
  if (symptoms)
    free ((char *) symptoms);
...
entries\nin the .ignored file.", sender);
  ...
  /* Walk through the valid requests */
  for (i = 0; i < MAX_COMMANDS; i++) {
    if (!strncmp (request, commands[i].name, strlen (request)) &&
		(int) strlen (request) > 2 && 
		! (disabled_commands & commands[i].mask)) {
...
\nThe files currently checked out are:",
		  ...
		  lpl_lock(LPL_WRITE,LPL_GLOBAL_REQUESTS,NULL);
		  OPEN_FILE (f, BATCH_FILE, "a", "action");
		  time_is = time (0);
...
void usage ()
{
  fprintf (stderr, "Usage: listproc [-1] [-e] [-n] {[-r <request>]}* \
{[-d <request>]}* {[-b <request>]}* [-B] [-f] [-p port] [-D]\n\
-1: Execute only once.\n\
-e: Echo reports to the screen.\n\
-n: Do not notify peer servers.\n\
...
-b: Batch 'request'.\n\
-B: Process the batch queue.\n\
-f: Perform AFD/FUI on archives.\n\
-p: Send a list of all published lists to the global-update-server.\n\
-D: Turn debug on.\n");
  exit (3);
}

